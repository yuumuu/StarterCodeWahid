window.Framework = {
    ...
    async processIncludes(html) {
        ...
        const regex = /<include\s+src=["']([^"']+)["']([^>]*)>(.*?)<\/include>|<include\s+src=["']([^"']+)["']([^>]*)\/>/gs;
        ...
        while ((match = regex.exec(processed)) !== null) {
            ...
            if (innerContent.trim()) {
                ...
                for (const [name, content] of Object.entries(slots)) {
                    ...
                    const targetRegex = new RegExp(`<slot\\s+name=["']${name}["']\\s*><\\/slot>|<slot\\s+name=["']${name}["']\\s*\\/>`, 'g');
                ...
                if (slots['default']) {
                    componentHtml = componentHtml.replace(/<slot\s*><\/slot>|<slot\s*\/>/g, slots['default']);
                }
            }
            
            // Cleanup unused slots in component
            componentHtml = componentHtml.replace(/<slot[^>]*><\/slot>|<slot[^>]*\/>/g, '');
    ...
    async processLayout(html) {
        const layoutRegex = /<layout\s+name=["']([^"']+)["']\s*>([\s\S]*?)<\/layout>/;
        ...
        if (match) {
            ...
            for (const [name, slotContent] of Object.entries(slots)) {
                const targetRegex = new RegExp(`<slot\\s+name=["']${name}["']\\s*><\\/slot>`, 'g');
    ...
    sanitize(html) {
        return DOMPurify.sanitize(html, {
            ...
            ADD_ATTR: ['x-data', 'x-text', 'x-if', 'x-for', 'x-bind', 'x-on', 'src', 'name', '@click', ':class'] // Allow Alpine attributes
        });
    },
    ...
    async render(componentPath, targetId) {
        ...
        // 7. Initialize Alpine (if needed, but Alpine observes DOM automatically? 
        // No, if we replace innerHTML, Alpine might need to be told to rescan if it's not a component.)
        // Actually, x-data on the body or parent might not pick up new innerHTML automatically if it's a whole new tree.
        // We might need to wrap the content in a component or use Alpine.initTree.
        // But since we are inside #router-view, let's assume the page itself might have x-data.
        
        // If the page has <script> tags (component logic), we need to execute them.
